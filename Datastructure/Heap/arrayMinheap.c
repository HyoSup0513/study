...
MinHeap *createArrayMinHeap(int maxCount)
{
    return createMaxHeap(maxCount);
}
...
void insertMinHeap(MinHeap *pHeap, int value)
{
    int i = 0;
...
HeapNode *deleteArrayMinHeap(MinHeap *pHeap)
{
    HeapNode *pReturn = NULL;
    HeapNode *pTemp = NULL;
...
int main(int argc, char *argv[])
{
    ...
    if (pHeap1 != NULL)
    {
        insertMinHeap(pHeap1, 10);
        insertMinHeap(pHeap1, 20);
        insertMinHeap(pHeap1, 60);
        insertMinHeap(pHeap1, 15);
        insertMinHeap(pHeap1, 35);
        insertMinHeap(pHeap1, 45);
        insertMinHeap(pHeap1, 25);
        insertMinHeap(pHeap1, 20);

        printf("Min Heap:\n");
        display(pHeap1);

        pNode = deleteArrayMinHeap(pHeap1);
        if (pNode != NULL)
        {
